...
class Guesser:
    ...
    def __init__(self, initial_wordlist):
        ...
        options = ['b','g','y']
        self.results_list = [[a,b,c,d,e] for a in options for b in options for c in options for d in options for e in options]
    ...
    def create_new_search_space(self, guess, results, simulation = False):
        ...
        for letter in letter_dict.keys():
            def validate(cand):
                both_zero = self.count_e_in_str(letter, cand) == 0 and letter_dict[letter][0] == 0
                hard_limit_frequency_match = self.count_e_in_str(letter, cand) == letter_dict[letter][0] and letter_dict[letter][1] == True
                soft_limit_plausibility = self.count_e_in_str(letter, cand) >= letter_dict[letter][0] and letter_dict[letter][1] == False
                return both_zero or hard_limit_frequency_match or soft_limit_plausibility
    ...
    def update_search_space_educated(self, guess, results):
        for i in range(5):
            if results[i] == 'g':
                self.results_list  = [cand for cand in self.results_list if cand[i] == results[i]]
    ...
    def guess_educated(self, absurdle=False):
        ...
        for candidate in self.wordlist:
            ...
            for result in self.results_list:
                totals.append(self.create_new_search_space(candidate, result, simulation=True))
